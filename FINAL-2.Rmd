...
#Load the data set
#Load the data set
resettlement<-read.csv("resettlement.csv")

class(resettlement$Value)
resettlement$Value <- as.numeric(as.character(resettlement$Value))

names(resettlement)
names(resettlement)[1] <- "Country / territory of asylum/residence"
...
resettlement$democracy.resettle <- ifelse(resettlement$`Country / territory of asylum/residence` %in% c("Norway", "Iceland", "Sweden", "New Zealand", "Finland", "Ireland", "Denmark", "Canada", "Australia", "Switzerland", "Netherlands", "Luxembourg", "Germany", "United Kingdom of Great Britain and Northern Ireland", "Uruguay", "Austria", "Spain", "Costa Rica", "France", "Chile", "Portugal", "South Korea", "Japan", "United States of America", "Malta", "Estonia", "Israel", "Botswana", "Cape Verde", "Taiwan", "Czech Rep.", "Belgium", "Cyprus", "Italy", "Slovania", "Lithuania", "Latvia", "Greece", "South Africa", "Timor-Leste", "Slovakia", "Malaysia", "Trinidad and Tobago", "Columbia", "Panama", "Bulgaria", "Argentina"), 1, 0)


resettlement$democracy.origin <- ifelse(resettlement$Origin %in% c("Norway", "Iceland", "Sweden", "New Zealand", "Finland", "Ireland", "Denmark", "Canada", "Australia", "Switzerland", "Netherlands", "Luxembourg", "Germany", "United Kingdom of Great Britain and Northern Ireland", "Uruguay", "Austria", "Spain", "Costa Rica", "France", "Chile", "Portugal", "South Korea", "Japan", "United States of America", "Malta", "Estonia", "Israel", "Botswana", "Cape Verde", "Taiwan", "Czech Rep.", "Belgium", "Cyprus", "Italy", "Slovania", "Lithuania", "Latvia", "Greece", "South Africa", "Timor-Leste", "Slovakia", "Malaysia", "Trinidad and Tobago", "Columbia", "Panama", "Bulgaria", "Argentina"), 1, 0)
...
##Creating Factor Variables for Region of Resettlement
...
```{r Subset for previous USSR countries}
...
```{r Vietnam to entire resettlement }

#regression and plot for all of Veitname subset
Vietnam<-subset(resettlement, Origin == "Viet Nam")
...
```{r Myanmar to all of resettlement}
#regression and plot for all of Myanmar subset
Myanmar<-subset(resettlement, Origin == "Myanmar")
...
```{r MASS migration from all data to US}
## All refugees to the US
To.US.5000<-subset(resettlement, `Country / territory of asylum/residence` == "United States of America" & Value >= 5000)
head(To.US.5000)

To_US_fit_5000<-lm(Value ~ Year, data = To.US.5000)
To_US_fit_5000

plot(x=To.US.5000$Year, y = To.US.5000$Value,
     col = "red", frame = FALSE,
)
abline(To_US_fit_5000)

To_US_fit_5000_sum<-summary(To_US_fit_5000)
To_US_fit_5000_sum$r.squared
```

```{r Non-linear regression}
## All refugees to the US - non-linear fit
To.US.non.lin<-subset(resettlement, `Country / territory of asylum/residence` == "United States of America")
head(To.US.non.lin)

To_US_fit.sq<-lm(Value ~ Year + I(Year^2), data = To.US)
coef(To_US_fit.sq)

predict(To_US_fit.sq, newdata=list(Year=c(1980, 1981, 1982, 1983, 1984, 1985, 1986, 1987, 19988, 1989, 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015)))

Year.Values<-1980:2015
Year.Preds<-predict(To_US_fit.sq, newdata=list(Year=Year.Values))

plot(x=Year.Values, y = Year.Preds,
     col = "darkred", frame = FALSE,
)
abline(To_US_fit)
